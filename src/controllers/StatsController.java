...
public class StatsController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        CardsManager cardsManager = Controller.getCardsManager();
        AZDictionary = new Dictionary("AZ", new AlphabeticComparator(), cardsManager.getWords());
        ZADictionary = new Dictionary("ZA", new ReverseAlphabeticComparator(), cardsManager.getWords());
        ratioDictionary = new Dictionary("Ratio", new ReverseRatioComparator(), cardsManager.getWords());
        reverseRatioDictionary = new Dictionary("ReverseRatio", new RatioComparator(), cardsManager.getWords());
        ...
        tabPane.getSelectionModel().selectedItemProperty().addListener(
                (observableValue, tab, t1) -> {
                    switch (t1.getText()) {
                        case "Z-A" -> initZA();
                        case "INC Ratio" -> initReverseRatio();
                        case "DEC Ratio" -> initRatio();
                    }
                }
        );
    }
